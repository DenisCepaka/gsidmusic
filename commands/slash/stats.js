...
const command = new SlashCommand()
	.setName("stats")
	.setDescription("Dapatkan informasi tentang Klee")
	.setRun(async (client, interaction) => {
		...
		const statsEmbed = new MessageEmbed()
			.setTitle(`Informasi ${ client.user.username }`)
			.setColor(client.config.embedColor)
			.setDescription(
				`\`\`\`yml\nNama: ${ client.user.username }#${ client.user.discriminator } [${ client.user.id }]\nAPI: ${ client.ws.ping }ms\nWaktu Berjalan: ${ runtime }\`\`\``,
			)
			.setFields([
				{
					name: `Server stats`,
				...
				{
					name: "System stats",
					value: `\`\`\`yml\nRUNING at: ${ osver }\nUptime: ${ sysuptime }\nMachine name: DENISCEPAKA\nOperating System: Redmi Note 10 Pro\nProcessor: Qualcomm SM7150 Snapdragon 732G (8 nm)\nPage File: 5402MB used, 4627MB available\n\`\`\``,
					inline: false,
				},
			])
			.setFooter({ text: `Gamers Station ID` });
