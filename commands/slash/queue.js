...
const command = new SlashCommand()
	.setName("queue")
	.setDescription("Menunjukkan antrian saat ini")
	...
	.setRun(async (client, interaction, options) => {
		...
		if (!player.queue.size || player.queue.size === 0) {
			...
			const queueEmbed = new MessageEmbed()
				.setColor(client.config.embedColor)
				.setDescription(`**♪ | Sedang Memainkan:** [${ song.title }](${ song.uri })`)
				.addFields(
					{
						name: "Durasi",
						value: song.isStream
			...
			if (player.queue.size < 11 || player.queue.totalSize < 11) {
				...
				const embedTwo = new MessageEmbed()
					.setColor(client.config.embedColor)
					.setDescription(
						`**♪ | Sedang Memainkan:** [${ song.title }](${ song.uri }) [${ player.queue.current.requester }]\n\n**Antrian Lagu**\n${ pages[page] }`,
					)
					.addFields(
						{
							name: "Durasi Lagu",
					...
					.setFooter({
						text: `Halaman ${ page + 1 }/${ pages.length }`,
					});
				...
				const collector = interaction.channel.createMessageComponentCollector({
					filter: (b) => {
						if (b.user.id === interaction.user.id) {
							...
							return b
								.reply({
									content: `Hanya **${ interaction.user.tag }** yang dapat menggunakan tombol ini.`,
									ephemeral: true,
								})
								.catch(() => {
								});
