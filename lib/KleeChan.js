...
const webhookinfo = process.env.webhookinfo ? new WebhookClient({ url: process.env.webhookinfo }) : undefined;
const webhookgloballogs = process.env.webhookgloballogs ? new WebhookClient({ url: process.env.webhookgloballogs }) : undefined;
require("./KleePlayer");

class KleeChan extends Client {
	/**
	...
	build() {
        ...
        let LavaConnected = new MessageEmbed()
					.setColor("RED")
					//.setTitle("Connected !")
					.setDescription(`・KleeServer is connected.\n・All commands should now be working again!.`)
					 webhookinfo.send({ embeds: [LavaConnected] });
      })
			.on("nodeReconnect", (node) => {
				this.warn(
					`Node: ${ node.options.identifier } | Lavalink node is reconnecting.`,
				);
        let LavaReconnect = new MessageEmbed()
					.setColor("RED")
					//.setTitle("Reconnecting......")
					.setDescription(`・KleeServer is attempting reconnect to Server...\n`)
					 webhookinfo.send({ embeds: [LavaReconnect] });
      })
			.on("nodeDestroy", (node) => {
				this.warn(
        ...
        let LavaDisconnect = new MessageEmbed()
					.setColor("RED")
					//.setTitle("Disconnected. :(")
					.setDescription(`・KleeServer is disconnected.\n・KleeServer trying to reconnect.`)
					 webhookinfo.send({ embeds: [LavaDisconnect] });
      })
			...
			.on("trackError", (player, err) => {
				...
				let errorEmbed = new MessageEmbed()
					.setColor("RED")
					.setTitle("Kesalahan pemutaran!")
					.setDescription(`Gagal memuat trek: \`${ song.title }\``)
					.setFooter({
						text: "Ups! ada yang salah tapi itu bukan salahmu kak!",
					});
						...
						channel.send({
							embeds: [
								new MessageEmbed()
									...
									.setDescription(`Keluar dari Voice Channel <#${ oldChannel }>`),
							],
						})
            webhookgloballogs.send({
							embeds: [
								new MessageEmbed()
									.setColor(client.config.embedColor)
									.setDescription(`・Forced out of Voice Channel <#${ oldChannel }>\n・In the Server \n・**${ client.guilds.cache.get(player.options.guild) ? client.guilds.cache.get(player.options.guild).name : "a guild" }**`),
							],
						});
					}
      ...
      .setColor("GREEN")
      .setDescription(
      `・Player: ${ player.options.guild}\n・A wild player has been created in\n・**${ client.guilds.cache.get(player.options.guild) ? client.guilds.cache.get(player.options.guild).name : "Server"}**
`
    );
    webhookgloballogs.send({
    embeds: [LogEmbed],
      ...
      let playerDestroy = new MessageEmbed()
					.setColor("RED")
					.setDescription(`・Player: ${ player.options.guild}\n・A wild player has been destroyed in\n・**${ client.guilds.cache.get(player.options.guild)
						? client.guilds.cache.get(player.options.guild).name
						: "a guild"
					}**`)
					.setFooter({
					 ...
					 webhookgloballogs.send({ embeds: [playerDestroy] });
      })
			...
			.on("trackStart", async (player, track) => {
				...
				this.warn(
					`Player: ${ player.options.guild
						...
						let LogEmbed = new MessageEmbed()
      .setColor(client.config.embedColor)
      .setAuthor({ name: `Player: ${ player.options.guild
					}` })
      .setDescription(`・Track has been started playing\n・[${ track.title }](${ track.uri })\n・"Diminta Oleh" ${ track.requester }`)
    webhookgloballogs.send({
    embeds: [LogEmbed],
  })
				let trackStartedEmbed = this.Embed()
					.setAuthor({ name: "Sedang Memutar Musik...", iconURL: this.config.iconURL })
					.setDescription(`[${ track.title }](${ track.uri })` || "Tidak ada deskripsi")
					.setThumbnail("https://cdn.discordapp.com/attachments/955954026598506537/1002375634242240613/barbara-genshin-impact.gif")
					.addField("Diminta Oleh", `${ track.requester }`, true)
				...
				try {
					trackStartedEmbed.setImage(
						track.displayThumbnail("maxresdefault"),
					);
				} catch (err) {
					trackStartedEmbed.setImage(track.thumbnail);
				}
			...
			.on("queueEnd", async (player, track) => {
				...
				if (autoQueue) {
					...
					let queueEmbed = new MessageEmbed()
						.setColor(client.config.embedColor)
						.setAuthor({
							name: "Antrian telah berakhir",
							iconURL: client.config.iconURL,
						})
						.setFooter({ text: "Antrian telah berakhir" })
						.setTimestamp();
					client.channels.cache
						.get(player.textChannel)
									...
									client.channels.cache.get(player.textChannel).send({
										embeds: [
											new MessageEmbed()
												...
												.setDescription(
													`Klee keluar ya kak karena ga ada lagu lagi.\nMakasih kak!!.`,
												),
										],
									});
									player.destroy();
