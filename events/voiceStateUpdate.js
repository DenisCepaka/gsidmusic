			...
			if (player.get("autoPause") === true) {
				if (player.members === 1 && player.paused && player.prevMembers != player.members) {
					...
					let playerResumed = new MessageEmbed()
						...
						.setTitle(`Melanjutkan!`, client.config.iconURL)
						.setDescription(
							`Playing  [${ player.queue.current.title }](${ player.queue.current.uri })`,
						)
						.setFooter({ text: `Lagu saat ini telah dilanjutkan.` });
			...
			if (player.get("autoPause") === true) {
				if (
					...
					let playerPaused = new MessageEmbed()
						...
						.setFooter({
							text: `Lagu saat ini telah dijeda karena tidak ada seorang pun di Voice Channel`,
						});
